/*
 * BLDC_Controller1_sbs.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: BLDC_Controller1
 * Component Simulink Path: BLDC_Controller1
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                BLDC_Controller1_sbs
...
static RegMdlInfo rtMdlInfo_BLDC_Controller1[1] = { "",
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("BLDC_Controller1");
      mxArray *rhs[3];
      char * simulinkBlockPath = getSimulinkBlockPath(S);
      if (simulinkBlockPath == NULL) {
        ssSetErrorStatus(S,
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  slmrInitializeIOPortDataVectors(S, 3, 6);
  ...
  ssSetOutputPortSampleTime(S, 4, 1.0E-5);
  ...
  ssSetOutputPortSampleTime(S, 5, 1.0E-5);
  ...
  if (S->mdlInfo->genericFcn != (NULL) &&
    ...
    ssRegModelRefMdlInfo(S, "BLDC_Controller1", rtMdlInfo_BLDC_Controller1, 1);
  }
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 37, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 37)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
